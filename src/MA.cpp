...
#define Num 5										//种群大小
#define PSIZE	(Num+1)								//预留一个单元给offspring		
...
void Tabu_Search(int p1[], double SizeG[], double *cost)
{
	...
	*cost = f;
	...
	non_improve = 0;
	ys_best = f;
	while (iter<10000 && 1.0*(clock() - starting_time) / CLOCKS_PER_SEC  <  Time_limit)
	{
...
int main(int argc, char *argv[])
{
	...
	{		
		...
		printf("f[%d] = %lf \n", i, F[i]);	
		Out_results1(F[i], Output_File_Name, File_Name, Ctime[i]);
	}
	for (i = 0; i<Times; i++)
	{
		if (F[i] > F_best)
	...
	H_ave /= Times;
	AvgTime /= Times;
	Out_results(F_best, F_ave, F_worst, AvgTime, Output_File_Name, File_Name);
